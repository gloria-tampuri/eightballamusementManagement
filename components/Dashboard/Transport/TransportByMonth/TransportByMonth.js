...
const TransportByMonth = () => {
  ...
  const groupedDataByWeekAndMonth = data?.transport.reduce((result, expen) => {
    ...
    if (!result[monthName][weekNumber]) {
      result[monthName][weekNumber] = { transports: [], totalAmount: 0, weekStart, weekEnd };
    }
  ...
  if (groupedDataByWeekAndMonth) {
    totalYearAmount = Object.values(groupedDataByWeekAndMonth).reduce((total, weeksData) => {
      return total + Object.values(weeksData).reduce((weekTotal, { totalAmount }) => weekTotal + totalAmount, 0);
    }, 0);
  }
  ...
  return (
    <div className={classes.yearExpen}>
      ...
      <p className={classes.total}>Total Amount for the Year: {totalYearAmount}</p>

      {/* Display grouped transport data */}
      {groupedDataByWeekAndMonth &&
        Object.entries(groupedDataByWeekAndMonth).sort(([monthA], [monthB]) => {
          return monthNames.indexOf(monthB) - monthNames.indexOf(monthA);
        }).map(([monthKey, monthData]) => (
          <div key={monthKey}>
            <h2>{monthKey}</h2>
            {Object.entries(monthData).sort(([weekA], [weekB]) => weekB - weekA).map(([weekKey, weekData]) => (
              <div key={weekKey}>
                ...
                {weekVisibility[weekKey] && (
                  <div className={classes.list}>
                    <p className={classes.total}>Total Amount for Week {weekKey}: {weekData.totalAmount}</p>
                    <table>
                      <thead>
                        <tr>
                          <td>Date</td>
