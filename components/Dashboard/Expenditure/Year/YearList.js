...
const YearList = () => {
  ...
  const { data, error } = useSWR(`/api/expenditure/year/${year}`, fetcher, { refreshInterval: 1000 });
  ...
  return (
    <div className={classes.yearExpen}>
      ...
      {groupedDataByYear &&
        Object.entries(groupedDataByYear).map(([monthKey, { expenditures, totalAmount }]) => (
          <div className={classes.list} key={monthKey}>
            <h3 className={classes.month}>{monthKey}</h3>
            <p className={classes.total}>Total Amount for {monthKey}: {totalAmount}</p>
            <table>
              <thead>
                <tr>
                  <td>Date</td>
